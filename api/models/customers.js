...
module.exports = class Customers {
  ...
  static async findUser(username, email, excludeId = 0) {
    return new Promise((resolve, reject) => {
      const query =
        "SELECT * FROM customers WHERE (username = ? OR email = ?) AND id != ?";
  ...
  static getUpdateCustomers(customersId) {
    return new Promise((resolve, reject) => {
      connect.query(
        `SELECT customers.*,  
                COUNT(follow.follower_id) AS followersCount,
                COUNT(DISTINCT follow.follower_id) AS numberOfFollowers,
                COUNT(following.followed_id) AS followingCount
         FROM customers
         LEFT JOIN follow ON customers.id = follow.followed_id
         LEFT JOIN follow AS following ON customers.id = following.follower_id
